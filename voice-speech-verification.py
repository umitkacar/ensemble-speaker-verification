...
for i in range(len(voice_dir)):
    wav_filename = (
        data_path_save[0] + os.path.basename(voice_dir[i]).split(".")[0] + ".wav"
    )
...
embed_TPR, embed_FPR, embed_accuracy = calculate_results(
    embed_predictions, embed_labels
)
...
for i in np.arange(0, 1, 0.1):
    ...
    fusion_TPR, fusion_FPR, fusion_accuracy = calculate_results(
        fusion_predictions, fusion_labels
    )
...
fig.update_layout(
    ...
    font=dict(family="Arial, monospace", size=20, color="#000000"),
)
